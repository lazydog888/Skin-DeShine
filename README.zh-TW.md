...
GitHub 文件可透過頁首的 English / 繁體中文連結切換。支援自訂節點翻譯的新版 ComfyUI，可在 `Settings → Comfy → Locale → Language` 選擇 English 或 繁體中文；本專案的節點翻譯位於 `locales/en/` 與 `locales/zh-TW/`。語言選項不會改變 workflow、參數值或影像處理結果。
